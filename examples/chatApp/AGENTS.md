...
- Multi-session chat: `ChatSessions` resource (`list` / `create` / `update` / `delete`) + `ui::chat(:session($.active_session))`.
- Author component `SessionItem` emits `select(ChatSession)`; parent wires `:on(select => on_select)`.
- Active conversation uses `ui::section` + `ui::alert`; unselected state uses `ui::empty` on both surfaces.
