...
This example is a web-only `game::` program (ADR-012) demonstrating side-scrolling
platformer mechanics with Babylon WebGPU. Uses `game::movement :style(platformer)`,
`game::camera :mode(side_scroll)`, and `game::controller :scheme(arrows_jump)`.

### Platformer-specific nodes

- `game::sprite` — Billboard sprite from texture atlas with animation support
- `game::tilemap` — Tile-based level geometry from Tiled-compatible JSON
- `game::collectible` — Overlap-based pickups (coin, gem, health, powerup, key)
- `game::interactable` — Blocks that respond to bumps/hits (breakable, bumpable, switchable, container)
- `game::patrol` — 2D enemy AI patterns (walk_reverse, walk_fall, stationary, follow, flee)
- `game::warp` — Teleport triggers (pipes, doors) with direction requirements
- `game::level_end` — Level completion trigger with next-level loading

Do not add `app` / `component` / `resource` / `--terminal`.
