...
<!-- BEGIN SNOWFLOW_GAME_NOTE -->
## This project: WebGPU game

This example is a web-only `game::` program (ADR-012). Do not add `app` / `component` /
`resource` / `--terminal` dual-surface chrome. Edit intent in `main.silc` via `silc assist`;
runtime pixels live in the compiler `templates/game` tree, not hand-authored app `/src`.
<!-- END SNOWFLOW_GAME_NOTE -->
