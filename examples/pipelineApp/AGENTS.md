...
- Pipeline-only: no `app` / UI routes.
- `scrape::page` → `scrape::extract` → `tensor::tokenize` → `tensor::infer` (MiniLM CPU).
- Contract carries `raw_content` + `Emb384` vector field; persistence is synthesized.
- Run with `silc run main.silc --input-json '{"url":"https://…"}'`.
- Never hand-edit `.runtime/`.
